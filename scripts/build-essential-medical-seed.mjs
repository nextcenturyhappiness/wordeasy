...
import {
  assertEssentialSentenceQuality,
  buildEssentialContext,
  CLASSROOM_RE,
  patternKey
...
function buildContext(entry, index) {
  const built = buildEssentialContext(entry, index);
  assertEssentialSentenceQuality({
    lemma: entry.lemma,
    pos: entry.pos,
    ...
    context_sentence: built.sentence,
    plain_english_paraphrase: built.paraphrase,
    sentence_translation_zh: built.translation
  });
  if (BANNED_SENTENCE.test(built.sentence) || BANNED_SENTENCE.test(built.paraphrase)) {
    throw new Error(`Classroom frame survived for ${entry.lemma}: ${built.sentence}`);
  }
  return built;
}
...
function generateAdditiveRefresh(cards) {
  ...
  return [
    "-- Additive quality fix for 必备医学英语 clinical/physiological sentences.",
    ...
    "-- 20260910001600_essential_medical_roots_sentences.sql,",
    "-- or 20260910001700_essential_medical_clinical_sentences.sql.",
    "begin;",
    [
...
if (checkOnly) {
  ...
  const previousClinical = await readFile(previousClinicalUrl, "utf8");
  if (!previousClinical.includes("update public.contexts")) {
    throw new Error("Previous essential medical clinical-sentence migration is missing.");
  }
