...
import { FALLBACK_FRAMES, framesFor, patternKey } from "./lib/essential-medical-sentences.mjs";
...
const BANNED_SENTENCE = /opens with the .+ as a teaching example/iu;
...
function isUsableSentence(sentence, lemma) {
  return (
...
function buildContext(entry, index) {
  ...
  const chosen = ordered.find((frame) => isUsableSentence(frame.sentence(lemma), lemma));
  if (chosen === undefined) {
    throw new Error(`Could not build a unique context sentence for ${lemma}.`);
  }

  const sentence = chosen.sentence(lemma);
  const paraphrase = chosen.paraphrase(lemma, meaningEn);
  const translation = chosen.translation(zh);
...
function generateAdditiveRefresh(cards) {
  const senseRows = cards.map((card) =>
    tuple([sqlText(card.word_sense_id), sqlText(card.usage_note)])
  );
