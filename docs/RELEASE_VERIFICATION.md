...
The desktop web target uses the separate stable `wordeasy:desktop:v1:local-user` identity and the same independent deferred catalog/FSRS boundary. Initial JavaScript is 77.75 KiB gzip, Home cumulative JavaScript is 114.90 KiB gzip, and CSS is 3.33 KiB gzip. The desktop build emits no Web Manifest, Service Worker, Cloudflare `_headers`, Supabase environment value, or network/privileged Tauri plugin.
