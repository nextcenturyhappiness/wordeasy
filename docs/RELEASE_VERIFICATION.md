...
DEC-059 desktop web packaging again uses `wordeasy:desktop:v1:local-user`, emits no Web Manifest, Service Worker, Cloudflare `_headers`, Supabase origin, privileged secret, or network/privileged Tauri plugin, and keeps the 900-card catalog in a deferred chunk. Fresh gzip numbers belong with the desktop-web build check on the DEC-059 branch. The hosted cloud PWA remains a separate optional target.
