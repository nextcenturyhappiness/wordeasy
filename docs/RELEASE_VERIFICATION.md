...
DEC-059 (2026-09-22) restores a local-only personal Mac app and supersedes DEC-030 for that Tauri target. Progress lives in `wordeasy:desktop:v1:local-user` and is independent of both the old `article-english:cloud:${userId}` account and the phone `standalone:v1` database. Those existing databases are not migrated or deleted. The historical local-only DMG evidence above predates DEC-030 and is not a pass for the current desktop bundle. This Linux change did not rebuild or launch an Apple Silicon `.app`; TEST-042 live Mac execution remains Not verified until `npm run desktop:build` runs on Apple Silicon.
...
DEC-059 desktop web packaging again uses `wordeasy:desktop:v1:local-user`, emits no Web Manifest, Service Worker, Cloudflare `_headers`, Supabase origin, privileged secret, or network/privileged Tauri plugin, and keeps the catalog in a deferred chunk. That note recorded the pre-DEC-060 900-card catalog. DEC-060 merges the local catalog to Research 60 + Medical 731 and removes the third Home tile; a fresh desktop-web build check belongs with that change. The hosted cloud PWA remains a separate optional target.
