...
| DATA-004    |       P0 | Root               | explicit demo/preview/standalone/desktop/cloud modes, separate namespaces, visible local-only boundary, no fallback | Runtime/UI/build/E2E/secret tests                           | Automatically verified |
...
| DESKTOP-003 |       P0 | Root + Security    | strict Tauri CSP, zero capabilities, no privileged/network plugin or IPC command, remote navigation guard, no SW             | desktop build checker and Cargo navigation tests                            | Automatically verified |
| DESKTOP-004 |       P0 | Backend + QA       | stable `desktop:v1` IndexedDB identity, versioned 60-card catalog, frozen due Review queue, visible Mac-only boundary        | tests plus current-App two-module rating/full quit/restart persistence      | Manually verified      |
...
| PERF-006    |       P0 | Root + Content     | canonical 60-card dataset stays out of initial JS; demo uses 20-card subset, formal local targets use deferred 60-card chunk | production/standalone/desktop build checks                                  | Automatically verified |
...
| TEST-042    |       P0 | Root + QA          | renamed arm64 app/DMG, codesign/hdiutil/mount, current-App Research + Medical reveal/rating, full quit/restart pass                                                        | Manually verified      |
...
- Real Android Chrome and macOS Chrome PWA installation/standalone launch; the separate macOS App passed TEST-042.
