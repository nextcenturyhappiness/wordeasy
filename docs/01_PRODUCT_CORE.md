...
匹配字段：中文释义、英文 lemma / display form、语境句、搭配。已学/已复习卡片优先，但仍搜索本地词库，避免第一天搜索为空。desktop / standalone / personal 本地词库（Mac 个人版）在 lemma / displayForm 上允许有界编辑距离的拼写容错，中文释义仅在低成本单字近邻时容错（DEC-055）。cloud / PWA / demo 仍只做大小写不敏感的子串匹配，不得对纯拼写错误给出结果。
