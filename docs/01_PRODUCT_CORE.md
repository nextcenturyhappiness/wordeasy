...
- 目标 lemma，作为醒目标题直接可见；
- 一至两个科研或医学语境句子，目标词完整可见并可高亮，不得挖空；
- “这个词在当前语境中是什么意思？”；
- 可选 IPA、词性、模块和分类。

正面不得把目标词藏成填空。Reveal 后句子保持可见，目标词继续高亮。
...
`essential_medical` 是第三个顶层学习选项，显示名「必备医学英语」。词库覆盖 `data/essential-medical/lemmas.txt` 中的全部 lemma。每日分配 10 个新词，来自单一 `core`（课堂词汇）池。本模块 Context Card 不提供 collocations；能拆词根词缀时把课堂构词写入 `usage_note`，否则可空。例句为多样课堂/病历场景的 `original_example`，不得编造 DOI/PMID。Medical English 的 7 词根构词 + 3 病历用语配额不变。
...
```text
7 词根构词（morphology）
3 病历 / 课堂用语（chart / class vocabulary）
```
