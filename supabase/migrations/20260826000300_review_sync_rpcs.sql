create function public.ingest_review_events(p_events jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  loop
    begin
      ...
      v_canonical_revision := null;
      v_conflict_reason := null;

      if v_base_revision = v_observed_revision then
        if v_observed_revision = 0 then
          ...
          v_canonical_revision := 1;
        else
          update public.review_states as state
          set
            scheduler_state_json = v_scheduler_after,
            due_at = v_due_at,
            last_reviewed_at = v_ordering_at,
            revision = state.revision + 1,
            scheduler_implementation_version = v_scheduler_version,
            canonical_event_set_hash = null,
            updated_at = v_received_at
          where state.user_id = v_user_id
            and state.card_id = v_card_id
            and state.revision = v_base_revision
          returning state.revision into v_canonical_revision;
        end if;
        v_application_status := 'applied';
      else
        v_application_status := 'pending_reconciliation';
        v_conflict_reason := 'base_revision_mismatch';
      end if;
      ...
      v_results := v_results || jsonb_build_array(
        jsonb_build_object(
          ...
          'card_id', v_card_id,
          'status', case
            when v_application_status = 'applied' then 'applied'
            else 'conflict'
          end,
          'application_status', v_application_status,
          'canonical_revision', v_canonical_revision,
...
begin
  ...
  select
    ...
    coalesce((array_agg(page.received_at order by page.received_at desc, page.event_id desc))[1], p_after_received_at),
    coalesce((array_agg(page.event_id order by page.received_at desc, page.event_id desc))[1], p_after_event_id)
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'module', module.slug,
        'scheduler_state', state.scheduler_state_json,
        'due_at', state.due_at,
        'last_reviewed_at', state.last_reviewed_at,
        'revision', state.revision,
        'scheduler_implementation_version', state.scheduler_implementation_version,
        'canonical_event_set_hash', state.canonical_event_set_hash,
        'updated_at', state.updated_at
      )
      order by state.card_id
    ),
    '[]'::jsonb
  )
  into v_states
  ...
  where state.user_id = v_user_id
    and state.card_id in (
      select event.card_id
      from public.review_events as event
      where event.user_id = v_user_id
        and (
          (event.received_at, event.event_id) > (p_after_received_at, p_after_event_id)
          and (event.received_at, event.event_id) <= (v_next_received_at, v_next_event_id)
        )
      union
      select application.card_id
      from public.review_event_applications as application
...
create function public.get_reconciliation_bundle(p_card_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'review-card:' || v_user_id::text || ':' || p_card_id::text,
      0
    )
  );
  ...
  select
    coalesce(array_agg(event.event_id order by event.ordering_at, event.device_id, event.device_sequence, event.event_id), array[]::uuid[]),
    coalesce(
      jsonb_agg(
        jsonb_build_object(
  ...
  from public.review_events as event
  where event.user_id = v_user_id and event.card_id = p_card_id;
  ...
  from public.review_states as state
  where state.user_id = v_user_id and state.card_id = p_card_id;
...
create function public.commit_reconciled_review_state(
  p_card_id uuid,
...
begin
  ...
  select coalesce(
    array_agg(event.event_id order by event.ordering_at, event.device_id, event.device_sequence, event.event_id),
    array[]::uuid[]
  )
  into v_event_ids
  from public.review_events as event
...
revoke all on function public.pull_learning_changes(timestamptz, uuid, integer) from public, anon;
revoke all on function public.get_reconciliation_bundle(uuid) from public, anon;
...
grant execute on function public.pull_learning_changes(timestamptz, uuid, integer) to authenticated;
grant execute on function public.get_reconciliation_bundle(uuid) to authenticated;
grant execute on function public.commit_reconciled_review_state(
  uuid,
  bigint,
  text,
  jsonb,
