-- New-card eligibility uses learned_word_senses (first completed New),
-- not historical daily_assignments membership (DEC-057).
-- Additive replacement of ensure_daily_assignment_v1_unlocked;
-- ready sets stay frozen; empty shortage self-heal (DEC-048) unchanged.
...
begin
  ...
  select assignment_set.id, assignment_set.status
  into v_set_id, v_existing_status
  ...
  if v_set_id is not null and v_existing_status is distinct from 'shortage' then
  ...
  if p_module_slug = 'research_english' then
    ...
    loop
      ...
      where card.active
        ...
        and not exists (
          select 1
          from public.learned_word_senses as learned
          where learned.user_id = v_user_id
            and learned.module_id = v_module_id
            and learned.word_sense_id = sense.id
        );
  ...
  elsif p_module_slug = 'essential_medical' then
    v_category_slug := 'core';
    v_category_label := '必备医学英语';
    v_required := 10;
    select count(*)::integer
    into v_available
    from public.cards as card
    ...
    if v_available < v_required then
      ...
      values (
        ...
        v_required,
        v_available,
        'Not enough new 必备医学英语 cards are available.'
      )
      returning id into v_set_id;
  ...
  if v_set_id is not null then
    delete from public.daily_assignment_sets as assignment_set
    where assignment_set.id = v_set_id
      and assignment_set.user_id = v_user_id
      and assignment_set.module_id = v_module_id
      and assignment_set.study_date = p_requested_study_date
      and assignment_set.status = 'shortage'
      and assignment_set.assigned_count = 0;
  ...
  if p_module_slug = 'research_english' then
    ...
    select
      ...
      ranked.card_id,
      ranked.category_id,
      ranked.card_position::smallint
    from ranked
    where ranked.card_position <= 10
    order by ranked.card_position;
  else
    with ranked as (
      select
        card.id as card_id,
